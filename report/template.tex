...
% plot with checks -- MODIFY


% Model checks - draw samples from posterior predictive distribution - MODIFY report probabilities too refer to table in next
...
% Report table with top 5 counties
\begin{table}
\label{table:deaths}
...
% table with mortality rates -- MODIFY
\begin{table}
\label{table:probs}
...
% compare mortality rates for each county as in previous study -- MODIFY
According to the box plots in Figure , the distribution of mortality rate is far from being homogenous ??


% talk about overall infections and death number -- MODIFY
...
% report estimates in one table -- MODIFY
\begin{table}
\label{table:dic}
...
% table with probs from PPC -- MODIFY
\begin{table}
\label{table:dic}
\caption{Model fit assessment}
\begin{center}
...
% report predictive distribution checks -- MODIFY
...
% report comparisons table!!!! DIC, Gelfand-Ghost - MODIFY
